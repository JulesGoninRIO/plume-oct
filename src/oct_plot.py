...
def plot_quality_distribution(df, output_file, fig_name=None, bins_=100, log=False):
    """Based on the misalignment score, plot OCTs quality distribution.
    Args:
...
def plot_Bscans(volume, frames):
    ...
    fig.subplots_adjust(
        top=1, bottom=0, left=0, right=0.2 * len(frames), wspace=0.05, hspace=0
    )
...
def plot_fundus_from_path(oct_path: str, aspect = 1.0, title = None):
    """
    Load an OCT image and produces a figure the
    fundus by computing the mean of each A-scans given an image
...
def plot_normalization_comp(volume: np.ndarray, frame: int):
    ...
    fig.subplots_adjust(top=1, bottom=0, left=0, right=0.5, wspace=0.1, hspace=0.1)
    ax[0].imshow(volume[frame, :, :], cmap="gray")
...
def plot_superposition_frame(original_volume: np.ndarray,
    ...
    assert (
        len(original_volume.shape) == 3
    ), " Provided array has the wrong number of dimensions"
    assert (
        len(approximated_volume.shape) == 3
    ), " Provided array has the wrong number of dimensions"
    assert (
        frame >= 0 and frame < original_volume.shape[0]
    ), " frame to plot out of range, for original_volume"
    assert (
        frame >= 0 and frame < approximated_volume.shape[0]
    ), " frame to plot out of range, for approximated_volume"

    figure, axis = plt.subplots(1, 1, figsize=(15, 15))

    red_channel = np.concatenate(
        [original_volume[frame], original_volume[frame], approximated_volume[frame]],
        axis=1,
    )
    blue_channel = np.concatenate(
        [
            original_volume[frame],
            approximated_volume[frame],
            approximated_volume[frame],
        ],
        axis=1,
    )
    green_channel = np.concatenate(
        [
            original_volume[frame],
            np.zeros(original_volume[frame].shape),
            approximated_volume[frame],
        ],
        axis=1,
    )
    ...
    axis.text(
        rgb_image.shape[1] / 6 - 50, -10, "original image", fontsize=12, color="k"
    )
    axis.text(rgb_image.shape[1] / 3 + 75, -10, "superposition", fontsize=12, color="k")
    axis.text(
        rgb_image.shape[1] * 2 / 3 + 75,
        -10,
...
def plot_superposition(original_img: np.ndarray, approximated_img: np.ndarray):
    """Produces a figure of the superposition of a B-scan of volume A and a B-scan of volume B.
    B-scan A is shown on the left and is represented in the red channel in center image.
    B-scan B is shown on the right and is represented in the green channel in center image.
    ...
    assert (
        len(original_img.shape) == 2
    ), " Provided array has the wrong number of dimensions"
    assert (
        len(approximated_img.shape) == 2
    ), " Provided array has the wrong number of dimensions"
    assert (
        original_img.shape == approximated_img.shape
    ), "Images not of the same dimensions"

    figure, axis = plt.subplots(1, 1, figsize=(15, 15))

    red_channel = np.concatenate([original_img, original_img, approximated_img], axis=1)
    blue_channel = np.concatenate(
        [original_img, approximated_img, approximated_img], axis=1
    )
    green_channel = np.concatenate(
        [original_img, np.zeros(original_img.shape), approximated_img], axis=1
    )
...
def fundus_along_score(
    ...
    for score, label, color in zip(list_of_scores, labels, colors):
        ...
        axis.plot(
            [(i * width / 2) + width / 2 + 5 for i in score],
            [(i + n) * (height / (len(score) + 2 * n)) for i in range(len(score))],
            color=color,
            label=label,
        )
    ...
    plt.yticks(
        np.linspace(0, height, yticks),
        labels=np.linspace(0, volume.shape[0], yticks, dtype=int),
    )
...
def fundus_along_misalignment(
    ...
    if show_curve:
        axis.plot(
            [(i * (width / 2) * (1 / x_max)) + width / 2 + 5 for i in compound],
            [
                (i + n) * (height / (len(compound) + 2 * n))
        ...
        axis.plot(
            [(i * (width / 2) * (1 / x_max)) + width / 2 + 5 for i in compound_2],
            [
                (i + n) * (height / (len(compound_2) + 2 * n))
                for i in range(len(compound_2))
            ],
            color=color_2,
            label=label_2,
        )
        if snr_b_scans is not None:
            axis.plot(
                [(np.mean([snr_b_scans[i], snr_b_scans[i + 1]]) - np.min(snr_b_scans) )*10+ width / 2 for i in range(len(snr_b_scans)-1)],
                [
                    (i + n) * (height / (len(compound) + 2 * n))
                    for i in range(len(compound))
                ],
                color="red",
                label=f"(SNR - {np.round(np.min(snr_b_scans))})*10",
            )

    # Plot y axis by warpping it to the correct dimensions for easier readability
    plt.yticks(
        np.linspace(0, height, yticks), labels=np.linspace(0, size, yticks, dtype=int)
    )
    axis.get_xaxis().set_visible(False)
    ...
    if len(peak_list) > 0:
        ...
        plt.hlines(
            np.array([peak_list]) * (height / size) + 1,
            xmin=-100,
            xmax=1500,
            color="seagreen",
            alpha=1,
            label="detected_peaks",
            linestyles="dotted",
        )
...
def generate_gif(
    volume: np.ndarray, axis: int, aspect=0.0, start=0, end=0, step=1, name="mygif.gif"
):
...
def generate_gif_v2(
    volume: np.ndarray, axis: int, width=512, height=512, fps=1, name="mygif.gif"
):
    """
    Generate Gif sliceshow of OCT cube in chosen axis and stepping
    Args:
...
def plot_funduses(path_list: list, patient_id_list: list, aspect=1):
    ...
    Args:
        ...
        aspect (int, optional): ratio of fundus
        reconstruction, can be used to adat image shape into a square. Defaults to 1.
    """
    # Define figure placement
    rows = 2
    columns = len(path_list) // 2 + len(path_list) % 2
    fig, ax = plt.subplots(rows, columns, figsize=(10, 3), constrained_layout=True)

    assert len(patient_id_list) >= len(path_list), " Not enough patient ids provided"
