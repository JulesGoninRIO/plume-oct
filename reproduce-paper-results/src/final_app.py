...
class OCT_viewer(tk.Tk):
    ...
    def __init__(self):  # set up main window, tkinter widgets and menu
        ...
        self.menu.add_command(
            label="Compare annotations", command=lambda: self.compare_annotations()
        )
    ...
    def start(self):
        ...
        if not self.modality:
            ...
            if os.path.exists(
                self.folder_path[: self.index] + "annotation_" + modality + ".csv"
            ):
                if messagebox.askyesno(
                    title="Info",
                    message="A csv file with self.format "
                    + modality
                    + " already exists. Do you want to load it ?",
                ):
                    self.load(
                        self.folder_path[: self.index]
                        + "annotation_"
                        + modality
                        + ".csv"
                    )
            ...
            self.folder_path = (
                self.csv_path[: -self.csv_path[:-1][::-1].index("/") - 1]
                + "OCTs_"
                + self.modality
                + "/"
            )
    ...
    def load(self, path):
        ...
        self.bad = [
            [tk.IntVar(value=0) for j in range(int(self.modality))]
    ...
    def place_objects(self):
        ...
        self.oct, self.fundus, self.frame = (
            tk.IntVar(value=1),
            tk.IntVar(value=1),
            tk.IntVar(value=1),
        )
        ...
        nb_frames = len(
            [
                file
                for file in os.listdir(self.root_folder + self.oct_names[0])
                if file.endswith(".jpg")
            ]
        )
        if not self.modality or self.compare:
            self.bad = [
                [tk.IntVar(value=0) for j in range(nb_frames)]
        ...
        horizontal_scale = tk.Scale(
            ...
            length=300,
            bg="white",
            command=lambda x: self.change_frame(),
        )
        horizontal_scale.place(x=x, y=y)
        horizontal_scale_text = tk.Text(
            self, width=len("Change B-scan"), height=1, relief=tk.FLAT
        )
        ...
        vertical_scale = tk.Scale(
            self,
            variable=self.oct,
            from_=1,
            to=len(self.oct_names),
            orient="vertical",
            length=200,
            bg="white",
            command=lambda x: self.change_frame(),
        ...
        vertical_scale_text = tk.Text(
            self, width=len("Change OCT"), height=1, relief=tk.FLAT
        )
    ...
    def change_frame(self):
        ...
        self.button = tk.Checkbutton(
            text="Misalignment",
            variable=self.bad[self.oct.get() - 1][self.frame.get() - 1],
        )
        ...
        if not self.compare:
                ...
                for fundus in os.listdir(self.fundus_path[0]):
                    if fundus.find(oct_name) != -1:
                        ...
                        for k in range(2):
                            ...
                            image_left = ImageTk.PhotoImage(
                                image_left.resize((720, 720)).crop((0, 200, 720, 520))
                            )
            ...
            for k in range(2):
                if (
                    int(i)
                    in self.annotations[k][self.oct_names[self.oct.get() - 1][-36:]]
                ):
    ...
    def save(self):
        ...
        with open(
            self.folder_path[:index] + "/" + save_file, "w", newline=""
        ) as csvfile:
    ...
    def read_annotation(self, i):
        ...
        csv_path = filedialog.askopenfilename(
            title="Select directory", filetypes=[("CSV files", "*.csv")]
        )
        ...
        for row in df.iterrows():
            if row[1]["dataset_uuid"] in self.annotations[i].keys():
                ...
                figname = (
                    "_PID"
                    + str(row[1]["patient_id"])
                    + "_UUID"
                    + str(row[1]["dataset_uuid"])
                )
                if not os.path.exists(self.fundus_path[i] + figname + ".png"):
                    volume = np.array(
                        image_load_app.load_images_from_folder(row[1]["folder_path"])
                    )
                    fundus = oct_plot.compute_fundus(volume)
                    oct_plot.fundus_along_POC(
                        fundus,
                        compound_metric,
                        "compound",
                        x_max=max(np.max(compound_metric) + 10, 50),
                        peak_list=self.annotations[i][row[1]["dataset_uuid"]],
                        show_curve=False,
                    )
                    plt.savefig(self.fundus_path[i] + figname + ".png")
                    plt.close()

        self.label_left[i] = tk.Label(
            self.left_frame, image=ImageTk.PhotoImage(Image.open(self.image_path))
        )
